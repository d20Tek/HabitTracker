...
[TestClass]
public class UnmarkHabitCommandTests
{
    [TestMethod]
    public async Task ExecuteAsync_WithValidSettings_WritesSuccessMessage()
    {
        ...
        var command = new UnmarkHabitCompleteCommand(AnsiConsole.Console, habitService, userService);
        var context = CommandContextFactory.Create("mark-habit", ["3"]);
        var request = new UnmarkHabitCompleteCommand.Request { HabitId = "3" };
        ...
        // assert
        result.Should().Be(0);
        AnsiConsole.ExportText().Should().ContainAll("Success", "3", "decremented");
    }
    ...
    public async Task ExecuteAsync_WithInvalidCount_WritesErrorMessage()
    {
        ...
        var request = new UnmarkHabitCompleteCommand.Request { HabitId = "3", Date = DateTimeOffset.Now, Count = -1 };
